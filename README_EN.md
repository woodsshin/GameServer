...
- **Push Model replication**: Team-related replicated properties (`ABTKTeam::TeamID/TeamName/Members`, `BTKTeams`, `Team`/`TeamRole`, etc.) are registered under Push Model instead of default per-frame value comparison, reporting themselves dirty directly via `MARK_PROPERTY_DIRTY_FROM_NAME` at each write site. Points that edit an array element in place through a `Find*` pointer (role changes, status toggles, invitation acceptance) can't be picked up automatically by the engine, so each write site must mark it by hand.
